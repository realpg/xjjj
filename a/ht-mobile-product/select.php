...
<!--                </li>-->
        ...
        $sql="select * from mobile_product where mobile_product_title like '%$Searchname%' and mobile_product_level=$level order by mobile_product_sort desc,mobile_product_id desc";
        $count=$db->page_count("select count(*) from mobile_product where mobile_product_title like '%$Searchname%' and mobile_product_level=$level order by mobile_product_sort desc,mobile_product_id desc", 10);
...
<!--                               value="--><?//= $row['mobile_product_id'] ?><!--" onclick="Item(this, 'mmAll')">-->
<!--                    </td>-->
                    <td><?= $k + 1 ?></td>
                    <td style="text-align:left; padding-left:1%;">
                        <?= $row['mobile_product_title'] ?>
                        <?= $row['mobile_product_sort'] == 1 ? "<font color='red'><b>【置顶】</b></font>" : "" ?>
                        <?= $row['mobile_product_show'] == 1 ? "<font color='blue'><b>【推荐】</b></font>" : "" ?>
                    </td>
<!--                    <td><img src="../../--><?//= $row['product_logo'] ?><!--" width="100px"/></td>-->
                    <td><img src="../../<?= $row['mobile_product_image'] ?>" width="100px"/></td>
                    <td>
                        <a href="update.php?id=<?=$row['mobile_product_id']?>&level=<?=$level?>">
                            <img src="../images/t02.png" >
                        </a>
                    </td>
                    <td>
                        <a href="delete.php?id=<?=$row['mobile_product_id']?>&level=<?=$level?>">
