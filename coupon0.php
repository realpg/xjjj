...
<div class="container" style="background: <?=$color_index["color_content"]?>">
    ...
    <div class="t_main">
        <div class="t_nav_out" style="display: block;">
            ...
            <div class="work_hx" id="J_Eact2">
                ...
                <div class="work_in ">
                    <ul class="work_nav clearfix" id="tab_up">
                        <?php
                        $menu_rows=$db->query_lists("select menu_title from menu where menu_level=2 and menu_show=1 order by menu_sort asc,menu_id asc");
                        foreach ($menu_rows as $k=>$menu_row)
                        {
                            ?>
                            <li <?=$k==0?"class='work_now'":""?>><?=$menu_row["menu_title"]?></li>
                            <?php
                        }
                        ?>
                    </ul>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // // 头导航
        // var top = $('.t_nav').offset().top;
        // $(window).scroll(function(event) {
        //     //当浏览器卷去header的高时，让t_nav变成一个固定定位
        //     if($(window).scrollTop()>top){
        //         $('.t_nav').addClass('add_fixed');
        //         //为了让t_nav和t_con之间有间隙，给main设置margin-top
        //         $('.t_con').css('margin-top', 30);
        //     } else {
        //         //小于header的高时，让nav还原成一个标准流
        //         $('.t_nav').removeClass('add_fixed');
        //         $('.t_con').css('margin-top', 0);
        //     }
        // });
        // 底导航
